...
```powershell
# 편의 스크립트를 통한 실행 (80포트 → 실패 시 8080포트 자동 폴백)
.\.scripts\run-local.ps1
```

| 접속 주소 | 설명 |
|-----------|------|
| `http://localhost` | 포트 80 (관리자 권한 필요) |
| `http://localhost:8080` | 포트 8080 (일반 권한으로 실행 가능) |

> **참고**: 포트 80은 Windows에서 관리자 권한이 필요합니다. 스크립트가 자동으로 8080으로 재시도합니다.
