...
### 4. 메인 뷰 및 사이드 오프셋 레이아웃 (`.ui-panel`)
* **중앙 캔버스 강제 고정:** `game-arena`는 부모의 Flex Center를 받아 화면 정중앙에 위치하며, 내부에 포함된 패널들만 밖으로 벗어나는 구조.
* **상/하단선 동기화 아키텍처:**
  - `ui-panel`: `position: absolute; top: 0; bottom: 0; left: calc(100% + 40px)`를 부여해 중앙 게임창과 40px 간격으로 떨어지며 높이는 동일하게 연동됨.
  - 최상단의 '점수' 블록은 패널 최상단에, 최하단의 '도움말 버튼(`help-btn`)'은 Flex의 `margin-top: auto` 밀어내기 속성으로 패널 최하단 라인(게임 플레이 영역 하단선)에 한치의 오차 없이 정렬 보장.
